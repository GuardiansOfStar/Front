// src/components/ui/ReliableImage.tsx - fetchPriority 오타 수정
...
const ReliableImage = ({ 
  ...
  if (loadState === 'loading') {
    return (
      <div 
        className={`image-skeleton flex items-center justify-center ${className}`}
        style={style}
        onClick={onClick}
        aria-label={`${alt} 로딩 중`}
      >
  ...
  return (
    <img
      ...
      fetchPriority={isCritical ? 'high' : 'low'} // ✅ fetchPriority (camelCase)
      onLoad={() => {
        setLoadState('loaded');
        onLoad?.();
      }}
